...
class Fconv_PCA(nn.Module):     
    ...
    def __init__(self,  sizeP, inNum, outNum, tranNum=8, inP = None, stride = 1, padding=None, ifIni=0, bias=True, Smooth = True, iniScale = 0.4):
...
class Fconv_1X1(nn.Module):
    ...
    def __init__(self, inNum, outNum, tranNum=8, ifIni=0, bias=True, Smooth = True, iniScale = 0.4):
...
def MaskC(SizeP):     #改为了适应任意卷积核大小的mask
        p = (SizeP-1)/2
        x = np.arange(-p,p+1)/p
        X,Y  = np.meshgrid(x,x)
        C    =X**2+Y**2
...
class F_BN(nn.Module):
    def __init__(self,channels, tranNum=8):   # tranNum=8
